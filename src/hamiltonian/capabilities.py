...
MANIFESTS = {
    ...
    "openclaw": AdapterCapabilityManifest(
        schema=CAPABILITY_MANIFEST_SCHEMA,
        manifest_version=1,
        adapter_id="openclaw-local-contract",
        lane_id="openclaw",
        display_name="OpenClaw adapter",
        execution_kind="dry-run-contract",
        task_families=("analysis", "handoff"),
        strengths=(),
        capabilities=("sanitized-plan",),
        limitations=("No callable adapter is wired; gateway and remote delivery are explicitly unsupported.",),
        safety_controls=("no launch", "no gateway", "no remote delivery"),
        evidence_policy="packet-optional",
        local_only=True,
        remote_execution=False,
    ),
